...
// 에너지 단가는 지역 안 주유소들의 평균이라 소수점이 붙습니다. 근거란은 검토자가 같은 숫자로
// 다시 계산해 볼 수 있어야 하므로, 지급액과 달리 반올림하지 않고 적용한 값을 그대로 적습니다.
const wonExact = (n) => `${Number(n || 0).toLocaleString('ko-KR', { maximumFractionDigits: 2 })}원`;
...
const selfPricedFuels = new Set(['electric','hydrogen']);
...
// 사용자가 단가를 손으로 고쳤는지 기억합니다. 새 정산 건은 아직 저장 전이라 객체에 적어둘 수 없습니다.
let manualOilPrice = false;
...
// 음수나 잘못된 값이 들어오면 지급액이 깎이므로, 금액이 만들어지는 이 지점에서 한 번 더 막습니다.
...
function calculate(t) {
  ...
  const providedMeals = Math.min(days * 3, positive(t.mealProvided));
  const meals = Math.max(0, days * mealRate - providedMeals * (mealRate / 3));
  const cap = Number(lodgingCap(t.province) || 0) * positive(t.nights);
  const lodging = Math.min(positive(t.lodgingActual), cap);
  const vehicle = getVehicle(t.vehicleId);
  const fuel = t.transport === 'car' && vehicle?.efficiency ? positive(t.distance) / Number(vehicle.efficiency) * positive(t.oilPrice) : 0;
  const transit = ['train','bus','public'].includes(t.transport) ? positive(t.transitActual) : 0;
  const extras = t.transport === 'car' ? positive(t.toll) + positive(t.parking) : 0;
...
// 출장지는 사람이 읽는 이름으로 두고(정산서에 그대로 인쇄), 거리 조회용 주소는 숨겨 함께 보관합니다.
...
function summary(t) { const c=calculate(t), v=getVehicle(t.vehicleId); return `<div class="summary-grid"><div><div class="calc-list">
    ...
    <div class="calc-row"><div><b>${t.transport==='car'?'자가용 에너지비':t.transport==='official'?'관용차':'교통 운임'}</b><small>${t.transport==='car'?`${t.distance||0}km ÷ ${v?.efficiency||0}${v?.unit || efficiencyUnits[v?.fuel]} × ${wonExact(t.oilPrice)}/${energyUnits[v?.fuel] || '단위'}`:t.transport==='official'?'별도 운임 없음':'실제 결제금액'}</small></div><span>${t.transport==='car'?esc(t.oilSource):t.transport==='official'?'일비 50% 지급':'승차권 증빙'}</span><strong>${won(t.transport==='car'?c.fuel:c.transit)}</strong></div>
...
function bind() {
  ...
  $('[name="vehicleId"]')?.addEventListener('change',(e)=>{const vehicle=getVehicle(e.target.value);const trip=currentTrip();const priceInput=$('[name="oilPrice"]');
    ...
    const price=state.settings.fallbackFuel?.[vehicle?.fuel]||0;if(priceInput)priceInput.value=price;trip.oilPrice=price;trip.oilSource='기준단가';manualOilPrice=false;saveLocal();render();toast(`${vehicle?.name || fuelLabels[vehicle?.fuel]} 기준단가를 적용했습니다.`)});
  ...
  $('[name="oilPrice"]')?.addEventListener('input',()=>{manualOilPrice=true;const trip=currentTrip();delete trip.oilProof});
  $('[data-action="oil"]')?.addEventListener('click',lookupOil);
  $('[data-action="distance"]')?.addEventListener('click',lookupDistance);
  $('[name="destination"]')?.addEventListener('input',()=>{const hidden=$('[name="destinationAddress"]');if(hidden)hidden.value=''});
...
// 유가는 출발지 기준으로 조회합니다. 시·군·구까지 얻으면 그 평균이 실제 주유 지점에 더 가깝고,
// 못 얻으면 시·도 평균으로 내려갑니다. 출발지 자체를 못 읽으면 출장지 시·도로 되돌립니다.
async function originArea(t){const fallback={area:provinceCodes[t.province]||'',sigungu:t.city||'',label:`출장지 ${provinceName(t.province)}${t.city?` ${t.city}`:''}`};const origin=t.origin?.trim();if(!origin)return fallback;try{const url=workerUrl('/region');url.search=new URLSearchParams({query:origin}).toString();const data=await workerJson(url,'출발지 지역을 확인하지 못했습니다.');const area=provinceCodes[provinceKey(data.sido)];if(!area)throw new Error(`출발지 시·도를 알 수 없습니다: ${data.sido}`);const sigungu=data.sigungu||'';return{area,sigungu,label:`출발지 ${data.sido}${sigungu?` ${sigungu}`:''}`}}catch(error){return{...fallback,notice:`${error.message} 출장지 ${provinceName(t.province)} 기준으로 조회합니다.`}}}
async function lookupOil(){try{const t=serializeTrip(),v=getVehicle(t.vehicleId);if(selfPricedFuels.has(v?.fuel)){const input=$('[name="oilPrice"]');input?.focus();input?.select();currentTrip().oilSource='직접 입력';toast(`${v.name || fuelLabels[v.fuel]}는 오피넷 조회 대상이 아닙니다. 실제 충전단가(원/${energyUnits[v.fuel]})를 직접 입력해 주세요.`);return}const region=await originArea(t);const area=region.area;if(region.notice)toast(region.notice);const proxy=workerUrl('/opinet');const params={area,sigungu:region.sigungu||'',fuel:v?.fuel||'gasoline',date:t.startDate};let result;if(proxy){proxy.search=new URLSearchParams(params).toString();result=await workerJson(proxy,'오피넷 조회에 실패했습니다.')}else result=await request(`/api/opinet?${new URLSearchParams(params)}`);$('[name="oilPrice"]').value=result.price;const cur=currentTrip();cur.oilPrice=result.price;cur.oilSource=`${result.source} (${result.tradeDate}) · ${result.areaName||region.label}`;manualOilPrice=false;cur.oilProof={source:result.source,tradeDate:result.tradeDate,price:result.price,region:region.label,areaCode:result.areaCode,areaName:result.areaName||'',productCode:result.productCode,notice:result.notice||'',lookedAt:new Date().toISOString()};toast(result.notice||`${region.label} 기준 ${result.source} ${wonExact(result.price)}/${energyUnits[v?.fuel]||'L'}를 적용했습니다.`)}catch(e){const t=serializeTrip(),v=getVehicle(t.vehicleId);const price=state.settings.fallbackFuel?.[v?.fuel||'gasoline']||0;$('[name="oilPrice"]').value=price;currentTrip().oilSource='기준단가';toast(`${e.message} 기준단가 ${wonExact(price)}/${energyUnits[v?.fuel]||'L'}를 사용합니다.`)}}
// Static Map은 콘솔에서 따로 구독해야 합니다. 아직이면 지도 없이 그대로 진행합니다.
async function routeMapImage(from,to){
  try{
    const url=workerUrl('/staticmap');
    url.search=new URLSearchParams({origin:from,destination:to}).toString();
    const data=await workerJson(url,'지도 이미지를 받지 못했습니다.',1);
    return data.image||''
  }catch{return ''}
}
async function lookupDistance(){const result=$('#distance-result');try{const t=serializeTrip();if(!t.origin?.trim()||!t.destination?.trim())throw new Error('출발지와 상세 출장지를 모두 입력해 주세요.');result.textContent='네이버 지도에서 경로를 찾는 중…';const url=workerUrl('/route');url.searchParams.set('origin',t.origin);url.searchParams.set('destination',t.destinationAddress?.trim()||t.destination);const data=await workerJson(url,'거리 조회에 실패했습니다.');const input=$('[name="distance"]');input.value=data.roundTripKm;const trip=currentTrip();trip.distance=Number(data.roundTripKm);trip.origin=data.origin;trip.distanceSource=`네이버 지도 왕복 (${data.oneWayKm}km × 2)`;trip.distanceProof={origin:data.origin,destination:data.destination,oneWayKm:data.oneWayKm,roundTripKm:data.roundTripKm,lookedAt:new Date().toISOString()};result.textContent=`편도 ${data.oneWayKm}km · 왕복 ${data.roundTripKm}km을 적용했습니다.`;toast('네이버 지도 왕복 거리를 입력했습니다.');const map=await routeMapImage(t.origin,t.destinationAddress?.trim()||t.destination);if(map){trip.distanceProof.mapImage=map;result.textContent+=' 지도 이미지도 증빙에 저장했습니다.'}}catch(error){result.textContent=error.message;toast(error.message)}}
...
// 거리·유가는 외부 API에서 받은 값이라 근거를 남겨야 정산 검토가 됩니다.
// 조회 시각과 응답 내용을 그대로 인쇄해 사진대지 앞에 붙입니다.
function lookupProof(t,v){
  const when=(iso)=>iso?new Date(iso).toLocaleString('ko-KR'):'-';
  const rows=[];
  if(t.distanceProof){const d=t.distanceProof;rows.push(['네이버 지도 자동차 경로',[
    ['출발지',d.origin],['도착지',d.destination],
    ['편도',`${d.oneWayKm}km`],['왕복 적용',`${d.roundTripKm}km (편도 × 2)`],
    ['조회 시각',when(d.lookedAt)],['제공',"NAVER Maps Directions 5"]]]);}
  const mapImage=t.distanceProof?.mapImage?`<figure class="proof-map"><img src="${t.distanceProof.mapImage}" alt="출발지·도착지 지도"><figcaption>파란 마커 출발지 · 빨간 마커 도착지 (NAVER Static Map)</figcaption></figure>`:'';
  if(t.oilProof){const o=t.oilProof;rows.push(['오피넷 유가',[
    ['적용 단가',`${wonExact(o.price)}/${energyUnits[v?.fuel]||'L'}`],['조회 기준',o.source],
    // 값 쪽에 출발지·출장지 중 무엇을 썼는지가 이미 들어 있습니다.
    ['기준일',o.tradeDate],['기준 지점',o.region],
    // 시·군·구 확정 유가가 없으면 시·도 평균으로 내려갑니다. 실제로 어느 단위를
    // 적용했는지는 출발지와 따로 적어야 검토자가 근거를 그대로 확인할 수 있습니다.
    ['집계 지역',`${o.areaName||'-'}${o.areaCode?` (코드 ${o.areaCode})`:''}`],
    ['제품코드',o.productCode||'-'],['조회 시각',when(o.lookedAt)],
    ...(o.notice?[['비고',o.notice]]:[])]]);}
  if(!rows.length&&t.transport==='car')rows.push(['조회 기록 없음',[['거리·단가','자동 조회 없이 직접 입력한 값입니다.']]]);
  if(!rows.length)return '';
  return `<section class="lookup-proof"><h2>거리·단가 조회 근거</h2>${rows.map(([title,items])=>
